{
# @todo I'll probably want to group quite a lot of these config options
...
protected-mode = "yes"; # @todo consider `true` / `false`
...
enable-protected-configs = "no";  # @todo consider `true` / `false`
enable-debug-command = "no"; # @todo consider `true` / `false`
enable-module-command= "no"; # @todo consider `true` / `false`
...
mptcp = "no"; # @todo consider `true` / `false`
...
tls-client-cert-file = null;
tls-client-key-file = null;
tls-client-key-file-pass = null;
...
tls-ca-cert-file = null; # @note provide either one or the other
tls-ca-cert-dir = null;
...
tls-auth-clients = "yes"; # @todo 'yes', 'no' or 'optional'
...
#
# tls-replication yes
tls-replication = "no";  # @todo consider `true` / `false`

# By default, the cluster bus uses a plain TCP connection. To enable
# TLS for the bus protocol, use the following directive:
#
# tls-cluster yes
tls-cluster = "no";  # @todo consider `true` / `false`
...
tls-ciphers = "DEFAULT:!MEDIUM"; # @todo look up tls cipher syntax
...
tls-ciphersuites = "TLS_CHACHA20_POLY1305_SHA256"; # @todo look up tls cipher syntax

# When choosing a cipher, use the server's preference instead of the client
# preference. By default, the server follows the client's preference.
#
# tls-prefer-server-ciphers yes
tls-prefer-server-ciphers = "no";# @todo consider `true` / `false`
...
tls-session-caching = "yes";# @todo consider `true` / `false`
...
################################### RDMA ######################################
# @todo consider a top level `rdma.enable` option
...
rdma-port = null; # @note provide both port and bind
rdma-bind = null;
...
#
# supervised auto
supervised = "no"; # @note one of "no", "upstart", "systemd" or "auto"
...
loglevel = "notice"; # @note one of "debug", "verbose", "notice", "warning" or "nothing"
...
#
log-format = "legacy"; # @note one of "legacy" or "logfmt"
...
#
log-timestamp-format ="legacy"; # @note one of "legacy", "is08601" or "milliseconds"
...
syslog-enabled=  "no";  # @todo consider `true` / `false`

# Specify the syslog identity.
syslog-ident ="valkey";

# Specify the syslog facility. Must be USER or between LOCAL0-LOCAL7.
syslog-facility ="local0"; # @note one of "USER", "LOCAL0", "LOCAL1", "LOCAL2", "LOCAL3", "LOCAL4", "LOCAL5", "LOCAL6" or "LOCAL7"

# To disable the built in crash log, which will possibly produce cleaner core
# dumps when they are needed, uncomment the following:
#
# crash-log-enabled no
crash-log-enabled = "yes";  # @todo consider `true` / `false`

# To disable the fast memory check that's run as part of the crash log, which
# will possibly let the server terminate sooner, uncomment the following:
#
# crash-memcheck-enabled no
crash-memcheck-enabled = "yes";  # @todo consider `true` / `false`
...
stop-writes-on-bgsave-error ="yes";  # @todo consider `true` / `false`

# Compress string objects using LZF when dump .rdb databases?
# By default compression is enabled as it's almost always a win.
# If you want to save some CPU in the saving child set it to 'no' but
# the dataset will likely be bigger if you have compressible values or keys.
rdbcompression= "yes"; # @todo consider `true` / `false`
...
rdbchecksum ="yes"; # @todo consider `true` / `false`
...
# command.
rdb-version-check ="strict"; # @note one of "strict" or "relaxed"
...
sanitize-dump-payload ="no";  # @todo consider `true` / `false`
...
rdb-del-sync-files ="no"; # @todo consider `true` / `false`
...
#    HOST and LATENCY.
#
replica-serve-stale-data ="yes"; # @todo consider `true` / `false`
...
replica-read-only  ="yes"; # @todo consider `true` / `false`
...
# works better.
repl-diskless-sync ="yes"; # @todo consider `true` / `false`
...
#                       lose all your data.
repl-diskless-load ="disabled"; # @note one of "disabled", "swapdb", "on-empty-db" or "flush-before-load"
...
dual-channel-replication-enabled ="no"; # @todo consider `true` / `false`
...
# be a good idea.
repl-disable-tcp-nodelay ="no"; # @todo consider `true` / `false`
...
# connection.
repl-mptcp ="no"; # @todo consider `true` / `false`
...
repl-backlog-size = null; # @todo what's the default here?
...
propagation-error-behavior ="ignore"; # @note one of "ignore", "panic" or "panic-on-replicas"
...
replica-ignore-disk-write-errors ="no"; # @todo consider `true` / `false`
...
import-mode= "no"; # @todo consider `true` / `false`
...
replica-announced ="yes"; # @todo consider `true` / `false`
...
replica-annonce-ip = null; # @note defaults to the ip used by the websocket
replica-annonce-port = null; # @note defaults to the ip used by the websocket
...
user = {}; # @note type { user.${name} = { enable = true; ... }; }
...
aclfile = null; # @note incompatible with `user`
...
requirepass = null; # @note ignored with `aclfile`
...
acl-pubsub-default = "resetchannels"; # @note one of "allchannels" or "resetchannels"
...
#
# The default is:
maxmemory-policy = "noeviction"; # @note one of "volatile-lru", "allkeys-lru", "volatile-lfu", "allkeys-lfu", "volatile-random", "allkeys-random", "volatile-ttl" or "noeviction"
...
maxmemory-samples = 5; # @note type check range 1-64
...
maxmemory-eviction-tenacity = 10; # @note type check range 0-100
...
replica-ignore-maxmemory ="yes"; # @todo consider `true` / `false`
...
active-expire-effort = 1; # @note type check range 1-10
...
# way.
lazyfree-lazy-eviction ="yes";  # @todo consider `true` / `false`
lazyfree-lazy-expire ="yes"; # @todo consider `true` / `false`
lazyfree-lazy-server-del ="yes"; # @todo consider `true` / `false`
replica-lazy-flush ="yes"; # @todo consider `true` / `false`
...
lazyfree-lazy-user-del ="yes"; # @todo consider `true` / `false`
...
lazyfree-lazy-user-flush ="yes"; # @todo consider `true` / `false`
...
io-threads = null; # @note maybe a default of `1`, but I'm not sure
...
#           absolute values.
oom-score-adj = "no"; # @note one of "no", "yes", "absolute" or "relative"
...
oom-score-adj-values = {primary = 0; replica = 200; background = 800; }; # @note type check range -2000 - 2000
...
disable-thp ="yes"; # @todo consider `true` / `false`
...
appendonly = "no";  # @todo consider `true` / `false`
...
appendfsync = "everysec"; # @note one of "always", "everysec" or "no"
...
no-appendfsync-on-rewrite = "no"; # @todo consider `true` / `false`
...
#
# shutdown-timeout 10
shutdown-timeout = null; # @todo check whether 10 is the default here
...
shutdown-on-sigint = ["default"]; # @note one of "default", "save", "nosave", "now", "force", "safe" or "failover"
shutdown-on-sigterm = ["default"]; # @note type check "save" and "nosave" are not set together
...
cluster-enabled = "no"; # @todo consider `true` / `false`
...
cluster-config-file = null; # @todo consider the default
...
cluster-allow-replica-migration ="yes";  # @todo consider `true` / `false`
...
# option to no.
cluster-require-full-coverage ="yes";# @todo consider `true` / `false`
...
cluster-replica-no-failover ="no";# @todo consider `true` / `false`
...
cluster-allow-reads-when-down ="no";# @todo consider `true` / `false`
...
cluster-allow-pubsubshard-when-down ="no";# @todo consider `true` / `false`
...
cluster-link-sendbuf-limit = null;
...
cluster-announce-hostname = null;
...
cluster-announce-human-nodename = null;
...
cluster-preferred-endpoint-type = null; # @todo is "ip" a better default here?
...
cluster-slot-stats-enabled ="no";  # @todo consider `true` / `false`
...
cluster-slot-migration-log-max-len = null;
...
slot-migration-max-failover-repl-bytes = null;
